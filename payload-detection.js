export function detectPayloadMode(input, options = {}) {
  ...
  if (text) {
    ...
    // JSON content can itself be wrapped as a JSON string.
...
function detectContent(text) {
  ...
  if (looksLikeEscapedJsonEncoding(text)) {
    return { mode: 'json', confidence: 0.94, reason: 'content matches escaped JSON structure' };
  }
...
function normalizeForDetection(input) {
  ...
  // Ignore optional Markdown fences around examples.
...
function looksLikeEscapedJsonEncoding(text) {
  if (!/^[\[{]/.test(text)) return false;

  // Typical escaped JSON: {\"key\":...} or [\{"key":...}].
...
function unwrapStringPayload(text) {
  ...
  // Best-effort outer quote removal for strings that contain invalid
