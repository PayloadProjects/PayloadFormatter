...
const transported = formatJsonBestEffort(String.raw`{\"items\":[{\"description\":\"Alpha\\'s beta value\",\"beta\":\"Example\\&#174; Value\",\"gamma\":\"Value \\\\\\\" Units\"}]}`);
assert.equal(transported.valid, true);
assert.equal(transported.parsed.items[0].description, "Alpha's beta value");
assert.equal(transported.parsed.items[0].beta, 'Example&#174; Value');
assert.equal(transported.parsed.items[0].gamma, 'Value " Units');
...
const legitimateBackslashes = formatJsonBestEffort('{"value":"alpha\\\\beta\\\\gamma","message":"line1\\nline2","quote":"say \\\"hi\\\""}');
assert.equal(legitimateBackslashes.valid, true);
assert.equal(legitimateBackslashes.parsed.value, 'alpha\\beta\\gamma');
...
const wrappedXml = formatXmlBestEffort(JSON.stringify('<root><message>Alpha\\\'s beta value</message></root>'));
...
const xmlStructure = formatXmlBestEffort('<a>\n<b/>\n\n<!--note-->\n<d/>\n</a>');
assert.equal(xmlStructure.formatted, '<a>\n\t<b/>\n\t<!--note-->\n\t<d/>\n</a>',
  'XML formatting should use stable structural indentation and remove structural blank lines');

const xmlAttributes = formatXmlBestEffort('<p xmlns:x="urn:x"   a = "1"  b c=\'2\'><x/></p>');
assert.equal(xmlAttributes.formatted, '<p xmlns:x="urn:x" a="1" b c=\'2\'>\n\t<x/>\n</p>',
  'XML formatting should normalize opening-tag whitespace consistently');

const xmlEmpty = formatXmlBestEffort('<root><empty></empty><value>1</value></root>');
assert.equal(xmlEmpty.formatted, '<root>\n\t<empty/>\n\t<value>1</value>\n</root>',
  'XML formatting should normalize directly empty element pairs');
